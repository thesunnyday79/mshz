...
WORSHIP_PRESETS = {
    "✝ Deep Healing Worship (A major · 52 BPM)": {
        ...
        "desc":"Peace and surrender in God's presence. Grand piano + rain + 1111Hz healing undertone.",
    },
    ...
    "✨ Jesus Presence Healing (E major · 50 BPM)": {
        ...
        "desc":"Cinematic & intimate. Emotional piano at 50 BPM, glowing pads, spacious sound design.",
    },
...
def chord_pad(midi_notes, dur, vol=0.16):
    ...
    for midi in midi_notes:
        f = note_freq(midi)
        for d,w in zip([-0.0035,-0.0015,0.0,0.0015,0.0035],[0.45,0.80,1.00,0.80,0.45]):
...
def healing_undertone(dur, vol=0.016):
    ...
    tone = np.sin(2*np.pi*1111.0*t) * (0.82+0.18*np.sin(2*np.pi*0.07*t))
    return tone * vol
...
def generate_worship(cfg, duration_sec, reverb_strength, rain_vol, bell_on, piano_expression):
    ...
    while t_c < duration_sec-chord_dur*0.5:
        chord=prog[ci%len(prog)]; cd=chord_dur+beat*0.5
    ...
    while nt < duration_sec-beat*3:
        ...
        nd=float(rng.choice([beat,beat*1.5,beat*2.0,beat*2.5]))
    ...
    out = slow_fade(out, fade_sec=8.0)
    out = normalize(out)
    return to_wav(out)
...
# ══════════════════════════════════════════════════════════════════
OCEAN_PRESETS = {
    "🌊 Deep Ocean Sleep — 432Hz Theta Waves": {
        "wave_period": 9.0, "wind": 0.12, "depth": "deep",
        "piano": True,  "piano_key": "C_major", "bpm": 44,
        "prog": [(60,64,67),(67,71,74),(69,72,76),(65,69,72)],
        "melody": [60,62,64,67,69],
        "desc": "Deep ocean rumble + slow theta 432Hz piano. Dành cho ngủ sâu & thiền.",
    },
    "🏖 Gentle Beach Waves — Morning Calm": {
        "wave_period": 6.5, "wind": 0.06, "depth": "shallow",
        "piano": True,  "piano_key": "G_major", "bpm": 48,
        "prog": [(67,71,74),(74,78,81),(76,79,83),(72,76,79)],
        "melody": [67,69,71,74,76],
        "desc": "Sóng biển nhẹ nhàng + piano buổi sáng. Thức dậy bình an, giảm lo âu.",
    },
    "🌙 Midnight Ocean — Sleep & Relax": {
        "wave_period": 11.0, "wind": 0.18, "depth": "deep",
        "piano": False, "piano_key": None, "bpm": 42,
        "prog": [], "melody": [],
        "desc": "Chỉ tiếng sóng đêm khuya + gió biển + âm thanh dưới nước. Thuần thiên nhiên.",
    },
    "🐋 Underwater Meditation — Whale & Ocean": {
        "wave_period": 13.0, "wind": 0.05, "depth": "underwater",
        "piano": False, "piano_key": None, "bpm": 40,
        "prog": [], "melody": [],
        "desc": "Âm thanh đại dương sâu thẳm + whale song analog + sóng áp suất nước.",
    },
    "🌅 Sunset Cove — Piano & Ocean Blend": {
        "wave_period": 7.5, "wind": 0.09, "depth": "shallow",
        "piano": True,  "piano_key": "D_major", "bpm": 46,
        "prog": [(62,66,69),(69,73,76),(71,74,78),(67,71,74)],
        "melody": [62,64,66,69,71],
        "desc": "Hoàng hôn bên bờ biển. Piano D major + sóng biển chiều tà. Chữa lành cảm xúc.",
    },
}
...
def ocean_waves(dur, wave_period=8.0, depth="shallow", wind_vol=0.10, ocean_vol=0.70):
    ...
    # Layer 1: deep rumble (very low freq body of ocean)
    k_deep = np.ones(int(SR*0.014)) / int(SR*0.014)
    deep = np.convolve(noise, k_deep, mode='same') * 0.60

    # Layer 2: mid surf (wave break)
    k_mid = np.ones(int(SR*0.0025)) / int(SR*0.0025)
    surf = (np.convolve(noise, k_mid, mode='same') -
            np.convolve(noise, k_deep, mode='same')) * 0.30

    # Layer 3: hiss / foam / air
    k_hi = np.ones(int(SR*0.0004)) / int(SR*0.0004)
    hiss = (noise - np.convolve(noise, k_hi, mode='same')) * 0.10

    raw = deep + surf + hiss

    # Wave rhythm: sin^2 rise, faster crash
    phase = (t % wave_period) / wave_period
    wave_shape = np.sin(np.pi * phase) ** 2
    # Secondary ripple
    phase2 = (t % (wave_period * 0.55)) / (wave_period * 0.55)
    wave_shape += np.sin(np.pi * phase2) ** 2 * 0.35
    # Never fully silent (always some ocean presence)
    wave_env = wave_shape * 0.65 + 0.35

    ocean = raw * wave_env

    # Depth character
    if depth == "deep":
        # Boost low rumble, attenuate high hiss
        k_lp = np.ones(int(SR*0.006)) / int(SR*0.006)
        ocean = np.convolve(ocean, k_lp, mode='same') * 1.3
    elif depth == "underwater":
        # Heavy lowpass (muffled, pressure feeling)
        k_uw = np.ones(int(SR*0.020)) / int(SR*0.020)
        ocean = np.convolve(ocean, k_uw, mode='same') * 1.6
        # Add low pressure "whomp" pulse
        whomp_f = 0.12 + 0.04*np.sin(2*np.pi*0.007*t)
        ocean *= 0.7 + 0.3*np.sin(2*np.pi*whomp_f*t)

    ocean = ocean / (np.max(np.abs(ocean)) + 1e-9)

    # Wind layer (separate filtered noise)
    if wind_vol > 0:
        wind_noise = rng.normal(0, 1, n)
        k_wind = np.ones(int(SR*0.001)) / int(SR*0.001)
        wind = np.convolve(wind_noise, k_wind, mode='same')
        wind_swell = 0.6 + 0.4*np.sin(2*np.pi*0.022*t + 1.2)
        wind = wind / (np.max(np.abs(wind))+1e-9) * wind_swell
...
def whale_song(dur, vol=0.10):
    """Analog whale-song: slowly sweeping low tones"""
    n = int(SR * dur)
    rng = np.random.default_rng(55)
    t = np.linspace(0, dur, n, endpoint=False)
    out = np.zeros(n)

    # Place a few whale calls
    t_w = float(rng.uniform(5, 15))
    while t_w < dur - 10:
        call_dur = float(rng.uniform(4, 9))
        cn = int(call_dur * SR)
        ct = np.linspace(0, call_dur, cn, endpoint=False)
        # Frequency sweep: 60-120Hz with slow vibrato
        f_sweep = 65 + 35 * np.sin(np.pi * ct / call_dur) + 8 * np.sin(2*np.pi*0.8*ct)
        phase = np.cumsum(2*np.pi*f_sweep/SR)
        call = (np.sin(phase)*0.6 + np.sin(phase*2)*0.25 + np.sin(phase*3)*0.10)
        # Envelope: swell in, hold, fade out
        env = np.sin(np.pi * ct / call_dur) ** 0.6
        call = call * env * float(rng.uniform(0.7, 1.0))
        pos = int(t_w * SR); end = min(pos+cn, n)
        out[pos:end] += call[:end-pos]
        t_w += call_dur + float(rng.uniform(8, 20))

    out = out / (np.max(np.abs(out))+1e-9+np.max(np.abs(out))<0.01)
    return out * vol

def underwater_ambient(dur, vol=0.08):
    """Deep low drone — pressure, depth, mystery"""
    n = int(SR * dur)
    t = np.linspace(0, dur, n, endpoint=False)
    # Slowly drifting fundamental
    f_drift = 48 + 6*np.sin(2*np.pi*0.018*t) + 3*np.sin(2*np.pi*0.007*t)
    phase = np.cumsum(2*np.pi*f_drift/SR)
    tone = (np.sin(phase)*0.55 + np.sin(phase*2)*0.28 +
            np.sin(phase*3)*0.12 + np.sin(phase*4)*0.05)
    swell = 0.50 + 0.50*np.abs(np.sin(2*np.pi*0.012*t))
    return tone * swell * vol

def ocean_piano(dur, cfg):
    """Sparse piano melody over ocean — very gentle, wide spacing"""
    if not cfg.get("piano") or not cfg.get("prog"): return np.zeros(int(SR*dur))
    rng = np.random.default_rng(33)
    n = int(SR*dur); out = np.zeros(n, dtype=np.float64)
    bpm=cfg["bpm"]; beat=60.0/bpm; chord_dur=beat*6  # slower than worship
    prog=cfg["prog"]; mel=cfg["melody"]

    # Sparse chord pads (very soft)
    t_c=beat*float(rng.uniform(3,6)); ci=0
    while t_c < dur-chord_dur*0.5:
        chord=prog[ci%len(prog)]; cd=chord_dur+beat
        pad=chord_pad(chord, min(cd, dur-t_c+1), vol=0.10)
        pos=int(t_c*SR); end=min(pos+len(pad),n)
        out[pos:end]+=pad[:end-pos]*0.55
        t_c+=chord_dur; ci+=1

    # Very sparse piano notes (leave lots of silence — "ocean breathing")
    nt=beat*float(rng.uniform(4,8))
    while nt < dur-beat*4:
        mid=mel[int(rng.integers(0,len(mel)))]
        nd=float(rng.choice([beat*2,beat*3,beat*4]))
        vel=float(rng.uniform(0.28,0.48))  # very soft
        note=piano_note(note_freq(mid), nd, vel, bright=0.70)
        pos=int(nt*SR); end=min(pos+len(note),n)
        if pos<n: out[pos:end]+=note[:end-pos]*0.50
        nt+=nd*0.90+beat*float(rng.uniform(2.5,6.0))  # big gaps
...
def generate_ocean(cfg, duration_sec, ocean_vol, wind_vol, piano_vol,
    ...
    # 2. Whale song / underwater ambient
    if cfg["depth"] == "underwater":
        uw = underwater_ambient(duration_sec, vol=0.10)
        out += uw[:n]
        if whale_on:
            wh = whale_song(duration_sec, vol=0.12)
            out += wh[:n]
    elif whale_on:
        wh = whale_song(duration_sec, vol=0.09)
        out += wh[:n]
    ...
    # 4. 963Hz healing undertone (ocean version — very subtle)
    t = np.linspace(0, duration_sec, n, endpoint=False)
    undertone = np.sin(2*np.pi*963.0*t) * 0.010
    undertone *= 0.80 + 0.20*np.sin(2*np.pi*0.05*t)
    out += undertone

    # 5. Light reverb (room/cave — not cathedral)
...
with tab1:
    ...
    st.markdown("*Piano chữa lành · 1111Hz healing undertone · Christian Worship · Deep Prayer*")

    preset_name = st.selectbox("🎹 Chọn preset", list(WORSHIP_PRESETS.keys()), key="wp")
    cfg_w = WORSHIP_PRESETS[preset_name]
    ...
    with st.expander("📋 Cấu trúc bài"):
        bpm=cfg_w['bpm']; bd=60/bpm
        st.markdown(f"""
**Tuning:** 432Hz · **BPM:** {bpm} · **Beat:** {bd:.2f}s · **1 chord per** {bd*4:.1f}s
...
""")
    ...
    if st.button("✦ TẠO NHẠC WORSHIP 432Hz ✦", use_container_width=True, key="btn_w"):
        with st.spinner(f"Đang soạn {preset_name} · {dur_w} phút ✝"):
        ...
        fname = preset_name.split("(")[0].strip()
        fname = ''.join(c for c in fname if c.isalnum() or c==' ').strip().replace(' ','_').lower()
    ...
    st.caption("✝ 432Hz · 1111Hz Healing Undertone · I–V–vi–IV Worship · *Be still, and know that I am God* — Ps 46:10")
...
with tab2:
    ...
    ocean_preset = st.selectbox("🌊 Chọn cảnh biển", list(OCEAN_PRESETS.keys()), key="op")
    cfg_o = OCEAN_PRESETS[ocean_preset]
    st.info(cfg_o["desc"])

    c5,c6 = st.columns(2)
    with c5: dur_o   = st.slider("⏱ Thời lượng (phút)", 1, 60, 10, 1, key="dur_o")
    with c6: ocean_v = st.slider("🌊 Âm lượng sóng biển", 0.3, 1.0, 0.80, 0.05, key="ocv")

    c7,c8 = st.columns(2)
    with c7: wind_v  = st.slider("💨 Gió biển",          0.0, 1.0, 0.5,  0.05, key="wv")
    with c8: piano_v = st.slider("🎹 Piano (nếu có)",
                                  0.0, 1.0,
                                  0.55 if cfg_o["piano"] else 0.0,
                                  0.05, key="pv",
                                  disabled=not cfg_o["piano"])

    c9,c10 = st.columns(2)
    with c9:
        whale_on = st.toggle("🐋 Whale Song / Underwater", value=True, key="whale")
    with c10:
        rev_o = st.slider("🔊 Reverb (hang động)",  0.1, 0.8, 0.35, 0.05, key="rev_o")
...
| 💨 Wind | Intensity: **{cfg_o['wind']*100:.0f}%** base |
| 🎹 Piano | **{'Có · ' + str(cfg_o.get('piano_key','')) if cfg_o['piano'] else 'Không (pure nature)'}** · {cfg_o.get('bpm','-')} BPM |
| 🐋 Whale/UW | Whale calls + underwater pressure tones |
| ✨ Undertone | 963Hz healing frequency (rất nhẹ) |
""")

    if st.button("🌊 TẠO NHẠC BIỂN CHO GIẤC NGỦ 🌊", use_container_width=True, key="btn_o"):
        with st.spinner(f"Đang tạo {ocean_preset} · {dur_o} phút 🌊"):
            audio_o = generate_ocean(cfg_o, dur_o*60, ocean_v, wind_v, piano_v,
                                     whale_on, rev_o)
        mb = len(audio_o)/(1024*1024)
        st.success(f"🌊 Hoàn thành · {dur_o} phút · {ocean_preset.split('—')[0].strip()} · {mb:.1f} MB")
        st.audio(audio_o, format="audio/wav")
        fname2 = ocean_preset.split("—")[0].strip()
        fname2 = ''.join(c for c in fname2 if c.isalnum() or c==' ').strip().replace(' ','_').lower()
